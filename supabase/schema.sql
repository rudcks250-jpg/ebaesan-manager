...
begin
  ...
  from (
    select sum(case when transaction_type = 'deposit' then amount else -amount end)
      ...
      as running_balance
    from public.prepaid_transactions where customer_id = p_customer_id
  ) ledger;
...
begin
  ...
  if p_transaction_id is null then
    ...
    set transaction_type = p_transaction_type, amount = p_amount,
...
begin
  ...
  from public.prepaid_transactions where id = p_transaction_id for update;
  if not found then raise exception '거래내역을 찾을 수 없습니다.'; end if;
